...
const ListAssetDetailPage = loadable(() => import('../assets/ListAssetDetailPage'));
...
const LibraryPageContent = () => {
  ...
  return (
    <Stack style={{ height: '100vh' }} fullWidth>
      {showSideBar && <Box style={{ width: 240, height: '100%' }} skipFlex>
        {!isEmpty(categories) && (
          <LibraryNavbar
            showSharedsWithMe
            labels={navbarLabels}
      ...
      <Box style={{ overflowY: 'scroll' }}>
        <Switch>
          ...
          <Route path={cleanPath(`${path}/video/:id`)}>
            <ListAssetDetailPage />
          </Route>
