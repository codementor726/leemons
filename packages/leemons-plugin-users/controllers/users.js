...
const { verifyJWTToken } = require('../src/services/users/jwt/verifyJWTToken');
...
async function disableUserAgent(ctx) {
  await userAgentsService.disable(ctx.request.body.userAgent);
  ctx.status = 200;
  ctx.body = { status: 200 };
}

async function activeUserAgent(ctx) {
  await userAgentsService.active(ctx.request.body.userAgent);
  ctx.status = 200;
  ctx.body = { status: 200 };
}
async function checkTokenValidity(ctx) {


  const can = await verifyJWTToken(ctx.request.body.token).catch(err=>{
    ctx.status = 401;
  ctx.body = { status: 401, logged: false };

  });

  if (can) { 
    const profiles = await usersService.profiles(can.id);

    if (profiles.length > 0) {      
      if (profiles[0].sysName == 'admin') {
        ctx.status = 200;
        ctx.body = { status: 200, logged: true };
        return
      }
    }
  }
